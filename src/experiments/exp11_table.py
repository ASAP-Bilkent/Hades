...
def load_and_organize_data(json_path):
    ...
    for run in runs:
        if 'avg_rmse' not in run or 'avg_psnr' not in run or 'avg_ssim' not in run or 'avg_lpips' not in run:
            continue
...
def generate_latex_table(organized_data):
    ...
    idlg_types = ['all_attacks']
    idlg_display = {
        'all_attacks': '\\textbf{Oracle/Zero/Random}'
    }
    ...
    num_cols = len(all_fusion_values) + 1
    ...
    def get_combined_values(dataset_data, fusion, metric, attack_types):
        
        values = []
        for attack_type in attack_types:
            if attack_type in dataset_data and fusion in dataset_data[attack_type]:
                data = dataset_data[attack_type][fusion]
                value = data[metric]
                formatted = format_value(value, metric)
                values.append(formatted)
            else:
                values.append("NA")
        return "/".join(values) if values else "NA/NA/NA"
            ...
            for metric in metrics:
                ...
                for fusion in all_fusion_values:
                    if fusion not in fusion_values:
                        ...
                        if idlg_type == 'all_attacks':

                            combined_value = get_combined_values(organized_data[dataset], fusion, metric, ['oracle', 'zero', 'rand'])
                            row_parts.append(combined_value)
                        else:
                ...
                if idlg_type == 'all_attacks':
                    ...
                    combined_value = get_combined_values(organized_data[dataset], f_minus_one, metric, ['oracle', 'zero', 'rand'])
                    row_parts.append(combined_value)
                else:
    ...
    caption = "Reconstruction Quality Metrics (RMSE, PSNR, SSIM, LPIPS) by Dataset and $|\\mathcal{F}_{HE}|$. "
    caption += "Each cell shows values for three attack methods: Oracle/Zero/Random "
    caption += "(ground truth initialization: theoretical maximum reconstruction, all zeros: common attack scenario, random initialization: common attack scenario). "
    ...
    latex_content.append("\\label{tab:recon_quality_combined}")
...
def main():
    ...
    results_dir = os.path.join(project_root, "data", "results", "exp11")
    print(f"Looking for files in: {results_dir}")
    
    exp11_files = glob.glob(os.path.join(results_dir, "exp11*.json"))
    
    print(f"Found {len(exp11_files)} files")
    
    if not exp11_files:
        print("No exp11 JSON files found. Checking runs.json for exp11 data...")
        ...
        if os.path.exists(general_runs_file):
            ...
            print("No suitable files found for exp11 data.")
            return
    else:
        exp11_files.sort(key=extract_timestamp, reverse=True)
    
        print("Available exp11 files (newest first):")
        for i, file in enumerate(exp11_files):
            formatted_name = format_filename(file)
            print(f"[{i}] {formatted_name}")
    
        try:
            selection = int(input(f"\nEnter file number (0-{len(exp11_files) - 1}): "))
            if selection < 0 or selection >= len(exp11_files):
                raise ValueError("Invalid selection")
            selected_file = exp11_files[selection]
        except (ValueError, EOFError):
            print("Invalid selection or no input provided, using the most recent file.")
            selected_file = exp11_files[0]
    ...
    if not organized_data:
        print("No exp11 data found in the selected file.")
        return
    ...
    if selected_file.endswith("runs.json"):
        output_file = os.path.join(output_dir, "exp11_table.tex")
