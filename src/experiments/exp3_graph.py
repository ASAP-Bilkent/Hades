...
def create_fhe_vs_time_plot(processed_data, output_dir):
    ...
    for entry in processed_data:
        layer_groups[entry['dims']].append(entry)
    ...
    for i, (layer_size, entries) in enumerate(sorted(layer_groups.items(), key=lambda x: (x[0] is None, x[0]))):
    ...
    output_file = os.path.join(output_dir, "fhe_vs_time.pdf")
...
def generate_latex_figure(fhe_plot, output_dir):
    latex_file = os.path.join(output_dir, "exp3_figures.tex")
    ...
    caption += "for different hidden layer sizes. Each line represents a different hidden layer configuration, "
    caption += "including no hidden layers."
    
    latex_content.append(f"\\caption{{{caption}}}")
    latex_content.append("\\label{fig:exp3_performance}")
